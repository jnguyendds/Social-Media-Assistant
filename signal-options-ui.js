(function(root){
  ...
  function scoreObj(option){return (option&&option.signalScore)||null;}
  function scoreLabel(option){const n=scoreObj(option)&&scoreObj(option).overallScore;return Number.isFinite(n)?`${n}/100`:'Not scored';}
  function scoreBreakdown(option){const sc=scoreObj(option);if(!sc)return '<p class="note">Not scored yet. Older options can still be used normally.</p>';const labels={compositionScore:'Composition',subjectPreservationScore:'Subject preservation',cleanupQualityScore:'Cleanup quality',distractionRemovalScore:'Distraction removal',aestheticScore:'Aesthetic',platformSuitabilityScore:'Platform suitability',brandConsistencyScore:'Brand consistency',technicalConfidenceScore:'Technical confidence'};return `<div class="score-breakdown">${Object.keys(labels).filter(k=>Number.isFinite(sc[k])).map(k=>`<div class="review-row"><span>${labels[k]}</span><b>${sc[k]}/100</b></div>`).join('')}<p class="note">Scores are directional/relative only and are not engagement guarantees.</p></div>`;}
  function reportPanel(option){const r=option&&option.optimizationReport||{};const list=(items,empty)=>items&&items.length?`<ul class="option-list">${items.map(i=>`<li>${esc(i)}</li>`).join('')}</ul>`:`<p class="note">${empty}</p>`;return `<details class="optimization-report"><summary>Optimization Report</summary><div class="sect-t">Removed items</div>${list(r.removedItems,'No removed items reported.')}<div class="sect-t">Preserved items</div>${list(r.preservedItems,'No preserved items reported.')}<div class="sect-t">Confidence</div><p class="note">${Number.isFinite(r.confidence)?r.confidence+'/100':'Not reported'}</p><div class="sect-t">Warnings</div>${list(r.warnings,'No warnings reported.')}</details>`;}
  ...
  function render(result,state={}){
    ...
    <div class="action-bar"><button class="cta" id="saveImageBtn" type="button">Save Image ⬇</button><button class="btn2" id="copyCaptionBtn" type="button">Copy Caption</button><button class="btn2" id="exportPackageBtn" type="button">Export Package</button></div>
    ${scoreBreakdown(option)}${reportPanel(option)}
      ...
      ${state.comparisonHtml||''}${verificationSummary(state)}${reviewControls(option,state)}${exportControls(state)}<div class="option-split"><div><div class="sect-t">Instant Preview <span>(Local Canvas edits)</span></div>${renderList(option.localAdjustments||[],formatAdjustment,'No local edits listed.')}</div><div><div class="sect-t">AI Enhancement <span>(ChatGPT handoff)</span></div>${genOps.length?renderList(genOps,formatGenerativeOperation,''): '<p class="note">No AI handoff required for this option — you can still send it to ChatGPT with your own instructions below.</p>'}${handoff?`<div class="instr">${esc(handoff)}</div>`:''}<label class="review-row" style="display:block"><span>Describe your edit (optional)</span></label><textarea id="customEditInstruction" placeholder="e.g. Remove the trash can in the background" style="width:100%;min-height:72px;padding:10px 12px;border-radius:10px;border:1px solid var(--border);background:var(--surface2);color:var(--text);font-size:13px;font-family:inherit;resize:vertical">${esc(state.customInstruction||'')}</textarea><button class="btn2" id="copyChatGptPrompt" type="button">Copy ChatGPT prompt</button><button class="btn2" id="sendClean" type="button">Send image to ChatGPT ↗</button></div></div>
      ...
      <details><summary>Details</summary><p class="note">Risk: ${esc(option.risk)} · Output: ${outputLabel(option)} · Status: ${esc(option.status)} · Prompt: ${esc(result.promptVersion||'unknown')}</p><pre class="details-json">${esc(JSON.stringify({format:result.format,subject:result.subject,score:option.signalScore||option.score,optimizationReport:option.optimizationReport,diagnostics:result.diagnostics||null,brandProfileInfluence:option.brandProfileInfluence||null},null,2))}</pre></details>
    </div>`;
  }
