(function(root){
  ...
  function normalizeProject(p){const out=Object.assign({},p,{projectVersion:p.projectVersion||PROJECT_VERSION,assetVersion:p.assetVersion||ASSET_VERSION,migrationVersion:p.migrationVersion||0,diagnostics:scrubDiagnostics(p.diagnostics)});if(Array.isArray(out.slides))out.slides=out.slides.map(function(s){return s&&s.project?Object.assign({},s,{project:Object.assign({},s.project,{diagnostics:scrubDiagnostics(s.project.diagnostics)})}):s;});return out;}
  async function importLegacyDataUrls(project,assetStore){
    ...
    async function one(container,key,role){if(!container||!isDataUrl(container[key]))return;if(container.assetId||container[role+'AssetId'])return;const blob=parseDataUrl(container[key]);const asset=await assetStore.putAsset({projectId,role,blob,mimeType:blob.type,name:container.name,width:container.width,height:container.height});container[key]=undefined;delete container[key];container.assetId=asset.assetId;container[role+'AssetId']=asset.assetId;changed=true;}
